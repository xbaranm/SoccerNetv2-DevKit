...
class OUR_ResNET_152(pl.LightningModule):
    def __init__(self, max_epochs=1000, temperature=0.7, batch_size=16, lr=1e-3, output="default", mirrored=False, **kwargs):
        ...
        self.lr = lr
        self.conv = timm.create_model('resnet152', pretrained=False, num_classes=0, global_pool='')
        ...
        # self.head = nn.Sequential(
        #     nn.Linear(2048*7*7*2, 2048*2),  # Adjust input size
    ...
    def forward(self, inputs):
        ...
        # out_1 = self.conv.forward_features(inputs_1).view(-1, 2048*7*7)         
        # out_2 = self.conv.forward_features(inputs_2).view(-1, 2048*7*7)         
