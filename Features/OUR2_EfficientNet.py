...
class OUR_EfficientNet(pl.LightningModule):
    def __init__(self, max_epochs=1000, temperature=0.7, batch_size=16, lr=1e-3, output="default", mirrored=False, **kwargs):
        ...
        #     nn.Conv2d(1280, 2560, 3, 2),
        #     nn.BatchNorm2d(2560),
        #     nn.ReLU(),
        #     nn.Conv2d(2560, 2560,  3),
        #     nn.BatchNorm2d(2560),
        #     nn.ReLU(),
        # )
        # self.fc_head = nn.Sequential(
        #     # nn.BatchNorm1d(2560*2),
        #     nn.Linear(2560*2, 1)
        # )
        ...
        # To disable wandb error of LazyModule
        # image_channels = 3
        # crop_height = crop_width = 224
        # dummy = torch.zeros([batch_size, 2*image_channels, crop_height, crop_width], dtype=torch.float64)
        # print('Running forward on dummy data to hack the LazyModule error from wandb')
        # self.forward(dummy)
    ...
    def _calculate_loss(self, batch, mode="train"):
        ...
        if self.hparams.mirrored:
            inputs, inputs_mir = inputs.split(6, dim=1) # Bx12xWxH -> 2x Bx6xWxH
